...
void calcPagesFromRoot(int u, vector<int> &pagesFromRoot, const vector<int> &pages,vector<int> &visited,vector<vector<int>> &AL,int page,vector<int> &CC) {
	bool hasChild = false;

	if (visited[u] == 1) { //there must be a cyclic dependency -> already saw it once so need to delete any culminatingConcepts that exist below this.
		visited[u]++;
		for (int v : AL[u]) {
	...
	if (visited[u] == 2) //already saw it twice; need to exit the cycle
		return;
...
int binLiftLCA(int u, int v,vector<int> &lvl,vector<vector<int>> &dp,vector<vector<int>> &AL) {
	...
	for (int i=0;i<LN;++i) {
		if (diff & i) //need to jump by this one
			u = dp[u][i];
	}
...
int main() {
	...
	unordered_set<int> culminatingConcepts;
	
	for (int u=0;u<n;++u) {
		if (!visited[u]) //new connected component
			calcPagesFromRoot(u,pagesFromRoot,pages,visited,AL,0,culminatingConcepts,false);
	}
