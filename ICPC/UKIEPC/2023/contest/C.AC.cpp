...
double doDP(int pi1, int p, int p_curr) {
    if (p_curr == N) {
        ...
        // take thsi one - if pi1 undefined - make it pi1, else make it pi2.
        // don't take this one - i.e. preserve pi1, pi2
...
// double printDP(int pi1, int p, int p_curr) {
//     if (p_curr == N) {
//         cout << 0.5 * abs(sin(posts[pi1] - posts[0])) << endl;
//         return 0.5 * abs(sin(posts[pi1] - posts[0]));
//     } else {
//         double take, this_tri;
//         if (p == 0) take = -INF;
//         else {
//             this_tri = 0.5 * sin(posts[p_curr] - posts[pi1]);
//             take = this_tri + doDP(p_curr, p -1, p_curr + 1);
//         }
//         double leave = doDP(pi1, p, p_curr + 1);

//         if (take > leave) {
//             cout << "take" << " " << p_curr  << " "  << pi1 << endl;
//             cout << this_tri << endl;
//             printDP(p_curr, p -1, p_curr + 1);
//         } else {
//             cout << "leave" << endl;
//             printDP(pi1, p, p_curr + 1);
//         }
//         return dp[pi1][p][p_curr];
//     }
// }
